...
def lookup():
    ...
    tx_rate = (tx_bytes - stats['tx']['bytes']) * (8.0 / 1e9)
    stats['tx']['Gbps'] = tx_rate
    stats['tx']['bytes'] = tx_bytes

    # Similarly for RX.
    rx_rate = (rx_bytes - stats['rx']['bytes']) * (8.0 / 1e9)
...
def main():
    RX_Gbps_THRESHOLD = 0.4  # 
    required_count = 2     # Require 2 consecutive seconds to trigger a change.
