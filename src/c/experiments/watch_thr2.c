...
#define ACTIVE_MS 1000  // considera attivo se aggiornato nell’ultimo secondo
...
int main(void) {
    ...
    fprintf(csv, "timestamp_ms,qp_id,qp_hex,bytes,start_ns,last_ns,last_bps,below_ns_accum\n");
    fflush(csv);
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (bpf_map_lookup_elem(map_fd, &cur_key, &stats) == 0) {
                unsigned long long last_ms = stats.last_ns / 1000000ULL;
                if (ts_ms - last_ms <= ACTIVE_MS) {
                    __u32 qp_host = ntohl(cur_key);
                    fprintf(csv, "%llu,%u,0x%08x,%llu,%llu,%llu,%llu,%llu\n",
                            ts_ms,
                            qp_host,
                            qp_host,
                            stats.bytes,
